...
Data data[] = {
    ...
    "mVolts", MV, 0, 0, 0, 0, 0, 0, mV, ANALOGVAR, AN0_PIN,0,
    "Temperatura exterior", TEMP_EXT, 0, 0, 0, 0, 0, 0, TC1047, ANALOGVAR, AN1_PIN,0
};
        ...
        if(data[var_idx].type==ANALOGVAR){
            switch(data[var_idx].variable){
                case MV:
                    ...
                    array_proc[MV]->push(data[var_idx].inst_value);                   
                    break;
                case TEMP_EXT:
                    data[var_idx].raw_value = analogRead(data[var_idx].pin);
                    data[var_idx].inst_value = _measure_unit_calc(var_idx, data[var_idx].sensor);
